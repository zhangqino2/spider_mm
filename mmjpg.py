...
chrome_path = r'C:\Users\zhangqi\AppData\Local\Google\Chrome\Application\chromedriver'
...
class spider(object):
    ...
    def get_pic_urls(self):
        driver = webdriver.Chrome(r'C:\Users\zhangqi\AppData\Local\Google\Chrome\Application\chromedriver.exe')
        for girl_url in self.girl_urls:
            driver.get(girl_url)
            time.sleep(1)
            driver.find_element_by_xpath('//em[@class="ch all"]').click()
            #---------------------测试以id来寻找也是有效的---------------------------
            #driver.find_element_by_id("opic").click()
            #-------------------------------------------------------------
            time.sleep(1)
            # 这里暂停3秒之后获取html的源代码
